...
void player::load_assets(){
  ...
  if( !(teleport = load_sample((modFolder + "/sfx/teleport.wav").c_str()))){
    abort_on_error( "Cannot find file /sfx/teleport.wav \n Please check your files and try again");
  }
  if( !(boxslide = load_sample((modFolder + "/sfx/boxslide.wav").c_str()))){
    abort_on_error( "Cannot find file /sfx/boxslide.wav \n Please check your files and try again");
  }
  if( !(electricity = load_sample((modFolder + "/sfx/electricity.wav").c_str()))){
    abort_on_error( "Cannot find file /sfx/electricity.wav \n Please check your files and try again");
  }
  if( !(lever = load_sample((modFolder + "/sfx/lever.wav").c_str()))){
    abort_on_error( "Cannot find file /sfx/lever.wav \n Please check your files and try again");
  }
  if( !(conveyorBelt = load_sample((modFolder + "/sfx/conveyorBelt.wav").c_str()))){
    abort_on_error( "Cannot find file /sfx/conveyorBelt.wav \n Please check your files and try again");
  }
  if( !(fallLeaves = load_sample((modFolder + "/sfx/fallLeaves.wav").c_str()))){
    abort_on_error( "Cannot find file /sfx/fallLeaves.wav \n Please check your files and try again");
  }
  if( !(fallWood = load_sample((modFolder + "/sfx/fallWood.wav").c_str()))){
    abort_on_error( "Cannot find file /sfx/fallWood.wav \n Please check your files and try again");
  }
  if( !(oof = load_sample((modFolder + "/sfx/oof.wav").c_str()))){
    abort_on_error( "Cannot find file /sfx/oof.wav \n Please check your files and try again");
  }
  if( !(wind = load_sample((modFolder + "/sfx/wind.wav").c_str()))){
    abort_on_error( "Cannot find file /sfx/wind.wav \n Please check your files and try again");
  }
  if( !(splash = load_sample((modFolder + "/sfx/splash.wav").c_str()))){
    abort_on_error( "Cannot find file /sfx/splash.wav \n Please check your files and try again");
  }
}
...
void player::logic( tile newTiles[][32]){
  ...
  for (int i = 0; i < 24; i++){
    for( int t = 0; t < 32; t++){
      if(newTiles[i][t].type == SOLID || newTiles[i][t].type == RIDEABLE || newTiles[i][t].type == PUSHABLE){
        if(collisionAny(x + 4, x + 36, t*40, t*40 + newTiles[i][t].image[0] -> w, y, y + 40, i * 40, i * 40 + newTiles[i][t].image[0] -> h) &&
      ...
      if(newTiles[i][t].type == CLIMBABLE || newTiles[i][t].type == PUSHABLE){
        if(collisionAny(x + 10, x + 30, t*40, t*40 + newTiles[i][t].image[0] -> w, y, y + 40, i*40, i*40 + newTiles[i][t].image[0] -> h)){
          canClimbUp2 = true;
        }
      }
      if(newTiles[i][t].type == CLIMBABLE || newTiles[i][t].type == PUSHABLE){
        if(collisionAny(x + 10, x + 30, t*40, t*40 + newTiles[i][t].image[0] -> w, y, y + 50, i*40, i*40 + newTiles[i][t].image[0] -> h)){
          canClimbDown2 = true;
        }
      }
      if(newTiles[i][t].type == SOLID || newTiles[i][t].type == PUSHABLE){
        if(collisionAny(x + 10, x + 30, t*40, t*40 + newTiles[i][t].image[0] -> w, y - 10, y, i*40, i*40 + newTiles[i][t].image[0] -> h)){
          canClimbUp = false;
        }
      }
      if(newTiles[i][t].type == SOLID || newTiles[i][t].type == PUSHABLE || newTiles[i][t].type == RIDEABLE){
        if(collisionAny(x + 10, x + 30, t*40, t*40 + newTiles[i][t].image[0] -> w, y, y + 50, i*40, i*40 + newTiles[i][t].image[0] -> h)){
          canClimbDown = false;
        }
      }
      if(newTiles[i][t].type != AIR && newTiles[i][t].type != DANGER && newTiles[i][t].type != NPC && newTiles[i][t].type != SWITCH && newTiles[i][t].type != SWITCH_ON){
        if(collisionAny(x + 10, x + 30, t*40, t*40 + newTiles[i][t].image[0] -> w, y, y + 40, i*40, i*40 + newTiles[i][t].image[0] -> h)){
          canJump = false;
        }
      }
      if(newTiles[i][t].type != AIR && newTiles[i][t].type != DANGER && newTiles[i][t].type != NPC && newTiles[i][t].type != SWITCH && newTiles[i][t].type != SWITCH_ON){
        if(collisionAny(x + 10, x + 30, t*40, t*40 + newTiles[i][t].image[0] -> w, y - yVelocity, y + 40 - yVelocity, i*40, i*40 + newTiles[i][t].image[0] -> h) &&
          collisionTop(i*40, i*40 + 40, y - yVelocity, y + 40 - yVelocity)){
          canJumpUp = false;
        }
      }
    }
  ...
  for (int i = 0; i < 24; i++){
    for( int t = 0; t < 32; t++){
      if(newTiles[i][t].type == SOLID || newTiles[i][t].type == CLIMBABLE || newTiles[i][t].type == PUSHABLE){
        if(collisionAny(x + 10, x + 30, t*40, t*40 + 40, y, y + 50, i*40, i*40 + 40) &&
          collisionTop(y, y + 50, i*40, i*40 + 40)){
          canFall = false;
          if(!collisionAny(x + 10, x + 30, t*40, t*40 + 40, y, y + 41, i*40, i*40 + 40) &&
  ...
  for (int i = 0; i < 24; i++){
    for( int t = 0; t < 32; t++){
      //Win
      if(collisionAny(x,x + 40,t*40, t*40 + 40,y,y + 40 , i*40, i*40 + 40) && newTiles[i][t].type == END_ON && switchOn == true){
      ...
      //Fan Movements
      if(collisionAny(x + 10, x + 30, t*40, t*40 + 40, y, y + 40, i*40, i*40 + 40)){
      ...
      if(collisionAny(x + 10, x + 30, t*40, t*40 + 40, y, y + 50, i*40, i*40 + 40)){
        ...
        //Right
        if(newTiles[i][t].value == CONVEYORLEFT && newTiles[y/40][x/40+1].type != SOLID){
          if(canMoveRight){
            play_sample(conveyorBelt,255,125,random( 900, 1000),0);
            x += 4;
            t += 1;
      ...
      //Check collisions
      if(collisionAny(x,x + 40,t*40, t*40 + 40,y,y + 40 , i*40, i*40 + 40)){
        //Teleport
        if(portalsActive){
