...
// Menu/GUI
BITMAP *levelSelectLeft, *levelSelectRight, *levelSelectNumber, *cursor[2], *menuselect, *menu, *menu_edit, *intro, *help;
BITMAP *miniMap;
...
void changeMap(){
  ...
  if(customRun || editMode){
    // Load changes
    ifstream read((modFolder + "/data/" + editLevels[customLevelOn].fileName[0]).c_str());
    if(fexists((modFolder + "/data/" + editLevels[customLevelOn].fileName[0]).c_str())){
      for (int i = 0; i < 24; i++){
        for( int t = 0; t < 32; t++){
      ...
      abort_on_error((("Cannot find map file, ") + (modFolder + "/data/" + editLevels[customLevelOn].fileName[0])).c_str() );
    }
    read.close();

    read.open((modFolder + "/data/" + editLevels[customLevelOn].fileName[1]).c_str());
    if(fexists((modFolder + "/data/" + editLevels[customLevelOn].fileName[1]).c_str())){
      for (int i = 0; i < 24; i++){
        for( int t = 0; t < 32; t++){
      ...
      abort_on_error((("Cannot find map file, ") + (modFolder + "/data/" + editLevels[customLevelOn].fileName[1])).c_str() );
    }
    read.close();
    ...
    if( gameScreen != 2){
      if( gameScreen == 3){
        FSOUND_Stream_Play(0,editLevels[customLevelOn].music);
      }
      FSOUND_Stream_Play(1,editLevels[customLevelOn].ambience);
    }
    ...
    if( gameScreen != 2){
      if( gameScreen == 3){
        FSOUND_Stream_Play(0,levels[levelOn].music);
      }
      FSOUND_Stream_Play(1,levels[levelOn].ambience);
    }
...
void setupGame(bool first){
  if(first){
    ...
    if( fexists((modFolder + "/data/config.txt").c_str())){
      ...
      abort_on_error( "Cannot find file /data/config.txt \n Please check your files and try again");
    }
    ...
    if(!(darkness = load_bitmap( (modFolder + "/images/backgrounds/darkness.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/backgrounds/darkness.png \n Please check your files and try again");
    }
    if(!(darkness_old = load_bitmap( (modFolder + "/images/backgrounds/darkness.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/backgrounds/darkness.png \n Please check your files and try again");
    }
    if(!(spotlight = load_bitmap( (modFolder + "/images/spotlight.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/spotlight.png \n Please check your files and try again");
    }
    if(!(intro = load_bitmap( (modFolder + "/images/intro.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/intro.png \n Please check your files and try again");
    }
    if(!(menu = load_bitmap( (modFolder + "/images/gui/menu.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/gui/menu.png \n Please check your files and try again");
    }
    if(!(menu_edit = load_bitmap( (modFolder + "/images/gui/menu_edit.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/gui/menu_edit.png \n Please check your files and try again");
    }
    if(!(menuselect = load_bitmap( (modFolder + "/images/gui/menuSelector.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/gui/menuSelector.png \n Please check your files and try again");
    }
    if(!(help = load_bitmap( (modFolder + "/images/gui/help.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/gui/help.png \n Please check your files and try again");
    }
    if(!(cursor[0] = load_bitmap( (modFolder + "/images/gui/cursor1.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/gui/cursor1.png \n Please check your files and try again");
    }
    if(!(cursor[1] = load_bitmap( (modFolder + "/images/gui/cursor2.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/gui/cursor2.png \n Please check your files and try again");
    }
    if(!(levelSelectLeft = load_bitmap( (modFolder + "/images/gui/levelSelectLeft.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/gui/levelSelectLeft.png \n Please check your files and try again");
    }
    if(!(levelSelectRight = load_bitmap( (modFolder + "/images/gui/levelSelectRight.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/gui/levelSelectRight.png \n Please check your files and try again");
    }
    if(!(levelSelectNumber = load_bitmap( (modFolder + "/images/gui/levelSelectNumber.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/gui/levelSelectNumber.png \n Please check your files and try again");
    }
    if(!(speech = load_bitmap( (modFolder + "/images/speech.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/speech.png \n Please check your files and try again");
    }
    if(!(speech_sign = load_bitmap( (modFolder + "/images/speech_sign.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/speech_sign.png \n Please check your files and try again");
    }
    if(!(error = load_bitmap( (modFolder + "/images/error.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/error.png \n Please check your files and try again");
    }
    if(!(back_mask = load_bitmap( (modFolder + "/images/back_mask.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/back_mask.png \n Please check your files and try again");
    }
    if(!(editBack = load_bitmap( (modFolder + "/images/backgrounds/editBack.png").c_str(), NULL))){
      abort_on_error( "Cannot find image /images/backgrounds/editBack.png \n Please check your files and try again");
    }

    //Load music
    music_menu = FSOUND_Stream_Open((modFolder + "/sfx/music/Menu.mp3").c_str(),2, 0, 0);

    //Load sound effects
    if(!(click = load_sample((modFolder + "/sfx/click.wav").c_str()))){
      abort_on_error( "Cannot find sound /sfx/click.png \n Please check your files and try again");
    }
    if(!(crumble = load_sample((modFolder + "/sfx/crumble.wav").c_str()))){
      abort_on_error( "Cannot find sound /sfx/crumble.png \n Please check your files and try again");
    }

    //Sets Font
    if(!(f1 = load_font((modFolder + "/fonts/arial.pcx").c_str(), NULL, NULL))){
      abort_on_error( "Cannot find font /fonts/arial.png \n Please check your files and try again");
    }
    ...
    if( fexists( (modFolder + "/data/levels.xml").c_str())){
      ...
      abort_on_error( "Cannot find file /data/levels.xml \n Please check your files and try again");
    }
    ...
    // Load levels
    for(xml_node<> *cLevel=allLevels-> first_node("level"); cLevel; cLevel=cLevel->next_sibling()){
      // Read xml variables
      ...
      bool lightingEnabled = convertStringToBool(cLevel-> first_node("lighting") -> value());
      bool particlesEnabled = convertStringToBool(cLevel-> first_node("particles") -> value());
      ...
      if( debugEnabled == true){
        // Draw to screen
        textprintf_centre_ex(screen,font,640,700,makecol(0,0,0),makecol(255,255,255),"Name:%s Id:%i Map:%s",name.c_str(),id, mapFile.c_str());
        textprintf_centre_ex(screen,font,640,760,makecol(0,0,0),makecol(255,255,255),"Image:%s %s Particles:%i",backgroundImage.c_str(),foregroundImage.c_str(),particlesEnabled);
        textprintf_centre_ex(screen,font,640,820,makecol(0,0,0),makecol(255,255,255),"Lighting:%i Music:%s Ambience:%s" ,lightingEnabled, musicFile.c_str(),ambienceFile.c_str());
        textprintf_centre_ex(screen,font,640,880,makecol(0,0,0),makecol(255,255,255),"Sign:%i Npc:%s" ,signTextRead.c_str(), npcTextRead.c_str());
      }
      ...
      if(fexists((modFolder + "/data/" + mapFile).c_str())){
        ...
        abort_on_error((("Cannot find map file, ") + (modFolder + "/data/" + mapFile)).c_str() );
      }
      ...
      if(fexists((modFolder + "/data/" + backMapFile).c_str())){
        ...
        abort_on_error((("Cannot find map file, ") + ("/data/" + backMapFile)).c_str() );
      }
      ...
      if( lightingEnabled != true && lightingEnabled != false){
        abort_on_error((("Unknown value in node: lightingEnabled for level ") + name).c_str() );
      }
      else{
      ...
      if( particlesEnabled != true && particlesEnabled != false){
        abort_on_error((("Unknown value in node: particlesEnabled for level ") + name).c_str() );
      }
      else{
      ...
      if( backgroundImage != "NULL"){
        if(!(levels[id].image[0] = load_bitmap((modFolder + string("/images/backgrounds/") + backgroundImage).c_str(), NULL))){
          abort_on_error((string("Cannot find image images/backgrounds/") + backgroundImage + string("\nPlease check your files and try again")).c_str());
        }
      }
      if( foregroundImage != "NULL"){
        if(!(levels[id].image[1] = load_bitmap((modFolder + string("/images/backgrounds/") + foregroundImage).c_str(), NULL))){
          abort_on_error((string("Cannot find image images/backgrounds/") + foregroundImage + string("\nPlease check your files and try again")).c_str());
        }
      }

      // Music/Ambience
      if( musicFile != "NULL"){
        if(!(levels[id].music = FSOUND_Stream_Open((modFolder + string("/sfx/music/") + musicFile).c_str(),2, 0, 0))){
        }
      }
      if( ambienceFile != "NULL"){
        if(!(levels[id].ambience = FSOUND_Stream_Open((modFolder + string("/sfx/ambience/") + ambienceFile).c_str(),2, 0, 0))){
        }
      }
    ...
    if( fexists( (modFolder + "/data/editLevels.xml").c_str())){
      ...
      abort_on_error( "Cannot find file /data/editLevels.xml \n Please check your files and try again");
    }
    ...
    // Load levels
    for(xml_node<> *cLevel=allEditLevels-> first_node("level"); cLevel; cLevel=cLevel->next_sibling()){
      // Read xml variables
      string name = cLevel-> first_node("name") -> value();
      int id = atoi(cLevel-> first_node("id") -> value());
      string mapFile = cLevel-> first_node("map") -> value();
      ...
      if(fexists((modFolder + "/data/" + backMapFile).c_str())){
        ...
        abort_on_error((("Cannot find map file, ") + (modFolder + "/data/" + backMapFile)).c_str() );
      }
      ...
      // Background/Foreground
      if( backgroundImage != "NULL"){
        if(!(editLevels[id].image[0] = load_bitmap((modFolder + string("/images/backgrounds/") + backgroundImage).c_str(), NULL))){
          abort_on_error((string("Cannot find image images/backgrounds/") + backgroundImage + string("\nPlease check your files and try again")).c_str());
        }
      }
      if( foregroundImage != "NULL"){
        if(!(editLevels[id].image[1] = load_bitmap((modFolder + string("/images/backgrounds/") + foregroundImage).c_str(), NULL))){
          abort_on_error((string("Cannot find image images/backgrounds/") + foregroundImage + string("\nPlease check your files and try again")).c_str());
        }
      }
      ...
      // Music/Ambience
      if( musicFile != "NULL"){
        if(!(editLevels[id].music = FSOUND_Stream_Open((modFolder + string("/sfx/music/") + musicFile).c_str(),2, 0, 0))){
         // allegro_message((string("Cannot find music soundtrack sfx/music/") + musicFile + string("\nPlease check your files and try again")).c_str());
         // exit(-1);
        }
      }
      if( ambienceFile != "NULL"){
        if(!(editLevels[id].ambience = FSOUND_Stream_Open((modFolder + string("/sfx/ambience/") + ambienceFile).c_str(),2, 0, 0))){
          //allegro_message((string("Cannot find ambience soundtrack sfx/ambience/") + ambienceFile + string("\nPlease check your files and try again")).c_str());
          //exit(-1);
        }
      }
    ...
    if( fexists( (modFolder + "/data/blocks.xml").c_str())){
      ...
      abort_on_error( "Cannot find file /data/blocks.xml \n Please check your files and try again");
    }
    ...
    // Load blocks
    for(xml_node<> *cBlock=allBlocks-> first_node("block"); cBlock; cBlock=cBlock->next_sibling()){
      // Read xml variables
      ...
      bool blockParticlesEnabled = convertStringToBool(cBlock-> first_node("particles") -> value());
      bool blockLightingEnabled = convertStringToBool(cBlock-> first_node("lighting") -> value());

      if( debugEnabled == true){
        // Draw to screen
        textprintf_centre_ex(screen,font,640,700,makecol(0,0,0),makecol(255,255,255),"Name:%s Id:%i Type:%s", blockName.c_str(),blockId, blockType.c_str());
        textprintf_centre_ex(screen,font,640,760,makecol(0,0,0),makecol(255,255,255),"Image:%s %s Particles:%i Lighting:%i",blockImage1.c_str(),blockImage2.c_str(),blockParticlesEnabled,blockLightingEnabled);
      }

      // Setup the blocks
      allTiles[blockId].setValues(blockId, blockType, 0, blockParticlesEnabled, blockLightingEnabled, blockName, blockImage1, blockImage2);
    }
...
void stepgo(){
  ...
  for (int i = 0; i < 24; i++){
    for( int t = 0; t < 32; t++){
      //Crumble blocks
      if( tiles[i][t].value == BLOCK1 || tiles[i][t].value == BLOCK2 || tiles[i][t].value == BLOCK3 || tiles[i][t].value == BLOCK4 || tiles[i][t].value == BLOCK5 || tiles[i][t].value == BLOCK6) {
        tiles[i][t].value += 1;
      }
      ...
      if( tiles[i][t].type == ENEMY){
        ...
        if(tiles[i + 1][t].type == AIR){
          ...
          if(tiles[i][t+1].type !=AIR || tiles[i+1][t+1].type == AIR || tiles[i][t+1].type == ENEMY){
            tiles[i][t].dir = LEFT;
            tiles[i][t].value -= 1;
          }
          else{
...
void game(){
  ...
  if( gameScreen == 1){
    ...
    if( !editMode){
      ...
      //Hover edit
      if(collisionAny(mouseX(),mouseX(),60,270,mouseY(),mouseY(), 700, 700 + 45)){
        if(newSelectorY != 700){
          newSelectorY = 700;
          selectorX = 60;
          play_sample(click,255,125,1000,0);
    ...
    for (int i = 0; i < 24; i++){
      for( int t = 0; t < 32; t++){
        ...
        //Make wind
        if(tiles[i][t].value == FAN && tiles[i-1][t].value == AIR||tiles[i][t].value == WIND && tiles[i-1][t].value == AIR){
          if(i > 0){
  ...
  if( gameScreen == 3){
    ...
    // Crumble block
    if(tiles[gameCharacter.getY()/40+1][gameCharacter.getX()/40].value == BLOCK){
      tiles[gameCharacter.getY()/40+1][gameCharacter.getX()/40].value = BLOCK1;
    }
    ...
    if(key[KEY_P] || joy[0].button[1].b){
      ...
      while( !key[KEY_P] && !key[KEY_ESC] && !joy[0].button[1].b){
        textout_centre_ex( buffer, font, "Paused press P to resume", 640, 440, makecol( 0, 0, 0), makecol(255,255,255));
...
void draw( bool toScreen){
  ...
  if(gameScreen==1){
      ...
      for( int t = 0; t < 32; t++){
        ...
        if( backTiles[i][t].image[0] != NULL) {
          ...
          if( backTiles[i][t].image[1] != NULL){
            draw_sprite( liveBuffer, backTiles[i][t].image[animationStep], backTiles[i][t].x, backTiles[i][t].y);
          }
          else if(backTiles[i][t].value != SPAWN){
            draw_sprite( liveBuffer, backTiles[i][t].image[0], backTiles[i][t].x, backTiles[i][t].y);
          }
          draw_trans_sprite( liveBuffer, back_mask, backTiles[i][t].x, backTiles[i][t].y);
          //Light
          if(backTiles[i][t].lightSource == true){
            draw_sprite(darkness, spotlight, backTiles[i][t].x + 20 - (spotlight->w/2), backTiles[i][t].y + 20 - (spotlight->h/2));
          }
        }
        ...
        if( tiles[i][t].image[0] != NULL) {
          if( tiles[i][t].image[1] != NULL){
             draw_sprite( liveBuffer, tiles[i][t].image[animationStep], tiles[i][t].x, tiles[i][t].y);
          }
          else if(tiles[i][t].value != SPAWN){
             draw_sprite( liveBuffer, tiles[i][t].image[0], tiles[i][t].x, tiles[i][t].y);
          }
          //Light
          if(tiles[i][t].lightSource == true){
            draw_sprite(darkness, spotlight, tiles[i][t].x + 20 - (spotlight->w/2), tiles[i][t].y + 20 - (spotlight->h/2));
          }
        }
        else{
    ...
    stretch_blit( liveBuffer, buffer, menu_view_x, menu_view_y, 640, 480, 0, 0, 1280, 960);
    ...
    if( editMode){
      textprintf_centre_ex(buffer,font,1168,73,makecol(0,0,0),-1,"%i",customLevelOn);
    }
    else{
  ...
  //Level editor
    ...
    for (int i = 0; i < 24; i++){
      for( int t = 0; t < 32; t++){
        // Back
        if( backTiles[i][t].image[0] != NULL) {
          draw_sprite( buffer, backTiles[i][t].image[0], backTiles[i][t].x, backTiles[i][t].y);
          draw_trans_sprite( buffer, back_mask, backTiles[i][t].x, backTiles[i][t].y);
        }
    ...
    textprintf_ex( buffer, font, 25, 25, makecol(0,0,0),-1, "Selected:%i", selected_object);
    stretch_sprite( buffer, cursor[0], mouseX(), mouseY(), 21 * resDiv, 26 * resDiv);

    if(key[KEY_S]){
      textprintf_centre_ex( buffer, font, 640, 20, makecol( 0, 0, 0), makecol(255,255,255), "Map Saved: %s, %s",
                           editLevels[customLevelOn].fileName[0].c_str(), editLevels[customLevelOn].fileName[1].c_str());
    }

    if(key[KEY_R]){
      textout_centre_ex(buffer, font, "Tiles Reset", 640, 20, makecol( 0, 0, 0), makecol(255,255,255));
    }
  }
  ...
  if(gameScreen == 3){
    ...
    //Paralax
    if( customRun){
      if(editLevels[customLevelOn].image[1] != NULL){//x 320, 960  y 240, 720
        stretch_sprite(buffer, editLevels[customLevelOn].image[1], 0, 0, 1280, 960);
      }
    }
    else{
      if(levels[levelOn].image[1] != NULL){
        stretch_sprite(buffer, levels[levelOn].image[1], 0 - (scrollX - 320)/2, 0 + (scrollY - 240)/2, 1280*2 - (scrollX - 320)/2, 960*2 + (scrollY - 240)/2);
      }
    }
    ...
    for (int i = 0; i < 24; i++){
      for( int t = 0; t < 32; t++){
        ...
        if( backTiles[i][t].image[0] != NULL) {
          //Images
          if( backTiles[i][t].image[1] != NULL){
            draw_sprite( buffer, backTiles[i][t].image[animationStep], backTiles[i][t].x, backTiles[i][t].y);
          }
          else if(backTiles[i][t].value != SPAWN){
            draw_sprite( buffer, backTiles[i][t].image[0], backTiles[i][t].x, backTiles[i][t].y);
          }
          ...
          //Light
          if(backTiles[i][t].lightSource == true){
            draw_sprite(darkness, spotlight, t * 40 + 20 - (spotlight->w/2), i * 40 + 20 - (spotlight->h/2));
          }
        }
        ...
        if( tiles[i][t].image[0] != NULL) {
          ...
          //Images
          if( tiles[i][t].image[1] != NULL){
            draw_sprite( buffer, tiles[i][t].image[animationStep], tiles[i][t].x, tiles[i][t].y);
          }
          else if(tiles[i][t].value != SPAWN){
            draw_sprite( buffer, tiles[i][t].image[0], tiles[i][t].x, tiles[i][t].y);
          }
          //Light
          if(tiles[i][t].lightSource == true){
            draw_sprite(darkness, spotlight, t * 40 + 20 - (spotlight->w/2), i * 40 + 20 - (spotlight->h/2));
    ...
    if( customRun){
      if(editLevels[customLevelOn].light == true){
        draw_sprite(darkness, spotlight, gameCharacter.getX() + 20 - (spotlight->w/2), gameCharacter.getY() + 20 - (spotlight->h/2));
    ...
    for (int i = 0; i < 24; i++){
      for( int t = 0; t < 32; t++){
        // Sign drawing
      	if(tiles[i][t].value == SIGN && collisionAny(gameCharacter.getX(),gameCharacter.getX() + 40,t*40, t*40 + 40,gameCharacter.getY(),gameCharacter.getY() + 40 , i*40, i*40 + 40)){
          BITMAP* tempSign;
          ...
          for( int line = 0; line < allLines.length(); line++){
            if( text_length(font, (allLines.substr( 0, line)).c_str()) > 700){
              ...
              textout_justify_ex(tempSign, font, allLines.substr( 0, endCharacter).c_str(), 80, 680, lineHeight, 600, makecol(0, 0, 0), -1);
              lineHeight += text_height(font);
              allLines.erase (0, endCharacter);
              line = 0;
            }
            else if( text_length(font, (allLines.substr( 0, allLines.length())).c_str()) < 700){
              textout_justify_ex(tempSign, font, allLines.substr( 0, allLines.length()).c_str(), 80, 680, lineHeight, 600, makecol(0, 0, 0), -1);
              allLines.erase (0, allLines.length());
            }
          }
		      stretch_sprite( buffer, tempSign, scrollX - 160, scrollY - 200, 380, 340);
		      destroy_bitmap( tempSign);
	      }

	      // NPC drawing
	      if(tiles[i][t].type == NPC && collisionAny(gameCharacter.getX(),gameCharacter.getX() + 40,t*40, t*40 + 40,gameCharacter.getY(),gameCharacter.getY() + 40 , i*40, i*40 + 40)){
          BITMAP* tempNpc;
          ...
          for( int line = 0; line < allLines.length(); line++){
            if( text_length(font, (allLines.substr( 0, line)).c_str()) > 700){
              int endCharacter = allLines.substr( 0, line).find_last_of(' ');
              textout_justify_ex(tempNpc, font, allLines.substr( 0, endCharacter).c_str(), 80, 680, lineHeight, 600, makecol(0, 0, 0), -1);
              lineHeight += text_height(font);
              allLines.erase (0, endCharacter);
              line = 0;
            }
            else if( text_length(font, (allLines.substr( 0, allLines.length())).c_str()) < 700){
              textout_justify_ex(tempNpc, font, allLines.substr( 0, allLines.length()).c_str(), 80, 680, lineHeight, 600, makecol(0, 0, 0), -1);
              allLines.erase (0, allLines.length());
            }
          }
		      stretch_sprite( buffer, tempNpc, scrollX - 160, scrollY - 200, 380, 340);
		      stretch_sprite( buffer, tiles[i][t].image[1], scrollX - 180, scrollY + 80, 80, 80);
    ...
    // Lighting
		circlefill( miniMap, gameCharacter.getX() / 8, gameCharacter.getY() / 8, 3, makecol( 255, 0, 0));
  ...
  if( toScreen){
    // Draw buffer
    if(gameScreen == 3){
      stretch_sprite( screen, buffer, -scrollX*2/resDiv + (SCREEN_W/2), -scrollY*2/resDiv + (SCREEN_H/2), SCREEN_W*2, SCREEN_H*2);
    }
    else{
  ...
  if( debugEnabled){
    textprintf_ex(screen,font,0,0,makecol(0,0,0),makecol(255,255,255),"FPS:%i",fps);
  }
}
