...
void tile::setValues( int newId, string newType, int newDir, bool newParticlesOn, bool newLightSource, string newName, string newImage1, string newImage2)
{
  // ID
  value = newId;
  ...
  // Image
  if( newImage1 != "NULL"){
    if(!(image[0] = load_bitmap((modFolder + string("/images/blocks/") + newImage1).c_str(), NULL))){
      allegro_message((string("Cannot find image /images/blocks/") + newImage1 + string("\nPlease check your files and try again")).c_str());
      exit(-1);
    }
  }
  if( newImage2 != "NULL"){
    if(!(image[1] = load_bitmap((modFolder + string("/images/blocks/") + newImage2).c_str(), NULL))){
      allegro_message((string("Cannot find image /images/blocks/") + newImage2 + string("\nPlease check your files and try again")).c_str());
      exit(-1);
    }
  }
  ...
  if( newParticlesOn != true && newParticlesOn != false){
    allegro_message((("Unknown value in node: newParticlesOn for block ") + name).c_str() );
    exit(-1);
  }
  else{
  ...
  if( newLightSource != true && newLightSource != false){
    allegro_message((("Unknown value in node: newLightSource for block ") + name).c_str() );
    exit(-1);
  }
  else{
